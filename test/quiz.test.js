...
describe('Quiz API', () => {
    ...
    // Test case to allow duplicate answer submission when flag is enabled
    it('should allow duplicate answer submission when flag is enabled', async () => {
        // Temporarily set the environment variable to allow duplicate answers
        process.env.ALLOW_DUPLICATE_ANSWERS = 'true';
        ...
        expect(answerRes2.statusCode).toEqual(200);

        // Reset the environment variable to its original value
        process.env.ALLOW_DUPLICATE_ANSWERS = 'false';
    });
    ...
    // Test case for non-existent quiz
    it('should return 404 for a non-existent quiz', async () => {
        const res = await agent.get('/api/quizzes/nonexistentid');
        expect(res.statusCode).toEqual(404);
        expect(res.body).toHaveProperty('message', 'Quiz not found');
    });
